...
$$
R(\tau) = \sum^T_{t=0} \gamma^t r_t \quad N \in [0, \infty)
$$ 
The horizon of turns over which the task has to be completed can either be finite or infinite. The total reward accumulated depends, as mentioned, on the discount factor $\gamma$ and the rewards that the agent can extract out of the environment based on its decisions (actions) when interacting with the environment. The aim of any RL technique is to maximize the expectation of this discounted total reward while navigating the environment. The cost function associated with such a scenario is
$$
...
>$$
>For a turn $t$, any $N$ actions are sampled from the policy to find out the expectation of the policy for that specific state
>$$
...
>$$
>Gradients are taken inside the expectation, trajectories are sampled and $R(\tau)$ is evaluated in order to determine the best policy.
>$$
