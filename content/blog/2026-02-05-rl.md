...
$$ 
Here, the policy $\pi_\theta$ has internal parameters $\theta$ that are learned in order to find the best trajectory that can maximize the reward collected from the environment. The initial probability of starting in state $s_0$, and the transition dynamics of the environment are also accounted for. In the probability corresponding to a trajectory $p_\theta (\tau)$, only the policy $\pi_\theta$ depends on $\theta$. For a trajectory, the total amount of rewards collected can be calculated as 
$$
...
>[!Info]
>The idea is to find the expectation over all the possible trajectories possible for the agent in the environment. For a setup with discrete actions and states this can be explicitly written out as:
>$$
>\sum_{s_0} \sum_{a_0} \sum_{s_1} \sum_{a_1} \dots {p_\theta(\tau)} {\color{yellow} R(\tau)}
>$$
>When everything is continuous, the same expectation over trajectories can be written as:
>$$
>\int \rho(s_0) \, ds_o \int \pi (a_0 \mid s_0) \, da_0 \int P(s_1 \mid s_0, a_0) \, ds_1 \dots {\color{yellow} R(\tau)}
>$$
>All of this is condensed into one compact notation representing the above given integral
>$$
...
$$ 
Since $p_\theta (\tau)$ is differential by $\theta$, and the gradient function $\nabla_\theta p_\theta(\tau)$ can be dominated by some other differentiable function $g$ such that $\int g \; d\mu < \infty$, the gradient can be swapped with the integral. This arises from a combination of [Leibniz's Theorem](https://en.wikipedia.org/wiki/Leibniz_integral_rule) for integral under partial differentiation and the [dominated convergence theorem](https://en.wikipedia.org/wiki/Dominated_convergence_theorem). The latter is ensured by the fact that any trajectory will be computed over a finite horizon $T$ and/or the rewards are discounted by a factor $\gamma < 1$ thereby diminishing the return and eventually making the series convergent and allowing the integrals and limits to commute. 
$$
...
>$$ 
>when $a(x)=a$ and $b(x)=b$, we will have a simpler resulting expression: $\int^b_a \dfrac{\partial}{\partial x}f(x, t) dt$
>This leads to the "trick" of putting the gradient, which is a derivative, under the integral sign. Additionally, the conditions 
